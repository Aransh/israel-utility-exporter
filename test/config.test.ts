...
test('REMOTE_WRITE_EXTRA_LABELS parses a comma-separated key=value list', () => {
  ...
  assert.deepEqual(config.remoteWrite?.extraLabels, { job: 'israel-utility-exporter', instance: 'exporter:9877' });
});
...
test('REMOTE_WRITE_EXTRA_LABELS defaults to an empty map when unset', () => {
  ...
  assert.deepEqual(config.remoteWrite?.extraLabels, {});
});
...
test('REMOTE_WRITE_EXTRA_LABELS trims whitespace around pairs and values', () => {
  ...
  assert.deepEqual(config.remoteWrite?.extraLabels, { job: 'israel-utility', env: 'prod' });
});
